...
export async function upsertHabitLog(habitId: string, periodo: string, estado: EstadoLog): Promise<HabitLog> {
    const row = await db
        ...
        .onConflict(oc => oc.columns(['habit_id', 'periodo']).doUpdateSet({ estado }))
        .returningAll()
        .executeTakeFirstOrThrow()
    return toDomainHabitLog(row)
}

/** Vuelve un día/período a "sin marcar" — borra el log en vez de guardar un estado, que es como se representa `null` en toda la app. */
